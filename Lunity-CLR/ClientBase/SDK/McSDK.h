...
static void initSDK() {
    ...
    ulong offsets[] = { 0xA8, 0x58, 0x38, 0x8 };
	localPlayer = evaluatePointer((ulong)gameBase + 0x02FFAF50, offsets);
    //log("Got local player!" + std::to_string((ulong)localPlayer.address));
}

static void updateSDK() {
    ulong offsets[] = { 0xA8, 0x58, 0x38, 0x8 };
    localPlayer.address = evaluatePointer((ulong)gameBase + 0x02FFAF50, offsets);
